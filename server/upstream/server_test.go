...
func TestServer_Register(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		s := NewServer(manager, nil, nil, log.NewNopLogger(), dbmanager.NewInMemoryDbManager())
		go func() {
...
func TestServer_Authentication(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		s := NewServer(manager, verifier, nil, log.NewNopLogger(), dbmanager.NewInMemoryDbManager())
		go func() {
			require.NoError(t, s.Serve(ln))
		}()
		defer s.Shutdown(context.TODO())
...
func TestServer_TLS(t *testing.T) {
	...
	s := NewServer(manager, nil, tlsConfig, log.NewNopLogger(), dbmanager.NewInMemoryDbManager())
	go func() {
		require.NoError(t, s.Serve(ln))
	}()
	defer s.Shutdown(context.TODO())
